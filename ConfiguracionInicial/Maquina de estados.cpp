...
EstadoPerro estadoPerro = QUIETO;
...
void Animation() {
	...
	animTime += 0.004f;
	if (AnimDog)
		estadoPerro = CAMINANDO;
	else
		estadoPerro = QUIETO;

	switch (estadoPerro) {
	case QUIETO:
		dogPos.z = FLegs = RLegs = head = tail = 0.0f;
		break;

	case CAMINANDO:
		dogPos.z += 0.00012f;

		FLegs = sin(animTime) * 12.0f;                    
		RLegs = -FLegs;

		head = sin(animTime) * 5.0f;              

		tail = sin(animTime) * 8.0f;              
		break;
	}
}
